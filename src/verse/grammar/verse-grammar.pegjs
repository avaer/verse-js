 ...
 * verse-grammar.pegjs
 *
 * Peggy grammar for a subset of the Verse programming language.
 * Vendorized from johanfortus/Verse-Online-Editor (MIT) and extended with
 * source-location tracking on statements (used for breakpoints, stepping,
 * and runtime error reporting).
 *
 * Regenerate the parser with: npm run build:parser
 */
...
// Every statement carries `loc` (Peggy location) so the interpreter can map
// execution back to source lines for breakpoints and error messages.
Statement
 = statement:StatementBody {
 if (statement && typeof statement === 'object' && !statement.loc) {
...
UsingDeclaration
 ...
 const declaration = UsingDeclaration(path);
