// semanticAnalysis.js
// Compile-time checks for the Verse subset: unknown identifiers/modules,
// failure-context enforcement for `decides` invocations (Verse error 3512),
// compound-assignment typing, and ToString-ability of interpolations.
// Vendorized from johanfortus/Verse-Online-Editor (MIT), extended to attach
// source locations to errors so the IDE can render them as Monaco markers.

import { getImportedSymbols, getLibrary, getSuggestedUsingForSymbol, resolveImportPaths } from './runtime/libraries.js';

export class SemanticError extends Error {
	constructor(message, code = 3506, loc = null) {
		super(message);
...
export function analyzeProgram(ast) {
	...
	for (const declaration of usingDeclarations) {
		if (!getLibrary(declaration.path)) {
			throw new SemanticError(`Unknown module '${declaration.path}'.`, 3506, declaration.loc || null);
		}
	}
...
function analyzeStatement(statement, scope) {
	try {
		analyzeStatementBody(statement, scope);
	} catch (error) {
		// Attach the innermost statement location so IDE markers point at the
		// offending line rather than the whole file.
		if (error instanceof SemanticError && !error.loc && statement.loc) {
...
function analyzeStatementBody(statement, scope) {
	switch (statement.type) {
	...
	for (const member of statement.members) {
		if (member.name?.name) {
			classScope.define(member.name.name, member.type === 'FunctionDeclaration'
				...
				: {
					type: member.type,
					verseType: resolveDeclaredType(member.varType || member.constType),
				});
		}
	}
...
function analyzeForStatement(statement, scope) {
	...
	loopScope.define(statement.variable.name, {
		type: 'LoopVariable',
		// Untyped range loop variables (`for (I := 1..5)`) are always ints.
		verseType: resolveDeclaredType(statement.varType) || { kind: 'primitive', name: 'int' },
	});
...
function ensureFailureContextForArrayAccess(expression, scope, failureContext) {
	...
	throw new SemanticError(DECIDES_CONTEXT_ERROR, 3512);
}
